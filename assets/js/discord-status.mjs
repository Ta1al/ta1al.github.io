export function validateDiscordWidget(payload, allowedGuildId) {
  if (
    !payload ||
  ...
  if (
    ...
    !invite.pathname.startsWith("/invite/")
  ) {
...
export async function fetchDiscordWidget(
  ...
  try {
    ...
    return validateDiscordWidget(await response.json(), allowedGuildId);
  } finally {
...
async function enhanceDiscordCard(card) {
  ...
  try {
    const widget = await fetchDiscordWidget(
      card.dataset.discordWidget,
      card.dataset.discordGuild,
    );
    ...
    presence.textContent =
      "Live presence is unavailable — the server is still open.";
